...
**第一層 資格篩選(6 條全過才進池,排地雷):** ①上市滿5年 ②近5年≥4年EPS正 ③近4季OCF正 ④負債比<60%(金融股 2800–2890 排除此條)⑤近60日日均成交金額>門檻 ⑥有最新財報。

**第二層 品質篩選(通過第一層者中標記,不淘汰):** ⑦近3年營收CAGR>10% ⑧近3年毛利率斜率≥0 ⑨近3年ROE均>15% ⑩盈餘修正動能(**僅標記,不列入「兩層全過」判定**,依原則等回測驗證後才加權)。

**估值旗標層(只加旗標、不淘汰任何標的):** 每檔給 🟢 合理偏低(PEG<1 且 前瞻PE<個股近5年PE中位)/ 🟡 一般 / 🔴 高估值警戒(前瞻PE>個股近5年PE的90百分位,或 PEG>2,或 前瞻PE>60x)。**PE 百分位用個股自己的歷史**,不用全市場平均。紅旗附「買進前請書面回答三題」警語;精華清單再依旗標細分 綠/黃/紅。
...
- **全部門檻寫在 `config/screener.yaml`**,不寫死;改完直接重跑 `screen.py`(**毋須重抓**)。
- **誠實原則**:任一條件因資料缺失無法判斷 → 標「⚠️資料不足」,**一律不當通過**。
- **省請求**:`deep_fetch_only_liquid` 讓未達流動性者不深抓財報;`refetch_after_days` 讓抓取**可續跑**。
- **額度**:免費版逐檔抓全市場(約 2,300 檔上市)較久,強烈建議設 `FINMIND_TOKEN`;`data/universe/` 已 gitignore(量大可重建)。
