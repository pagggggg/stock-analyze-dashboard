...
| 層 | 內容 |
| --- | --- |
| 第一層 · 狀態燈 | 🟢 無訊號級變化／🟡 有共識異動或 FCF 燈變色；紅燈保留給未來明確定義的高優先級同口徑訊號 |
| 第二層 · 訊號流水 | 只列**共識EPS 上下修 / FCF 品質燈變色**;**股價漲跌屬雜訊,刻意不列** |
...
- 人工指引逐筆保存金額/揭露狀態、日曆年/會計年度/單季口徑、方向、來源與日期；跨公司比較只採日曆年欄位。
...
> 只用**公開市場數據**做資格/品質研究,**無持倉或交易紀錄**;輸出僅供**縮小研究範圍**,**非買進清單**。

先把全市場資料抓一次存本地(FinMind 有頻率限制、且免費版不能一次抓全市場),之後篩選都讀本地:

```bash
# 1) 一次性:全市場逐檔抓 FinMind 存 data/universe/(可續跑;首次較久)
python fetch_universe.py                 # 全上市;或 --limit 30 / --stock-ids 2330,2454 測試

# 2) 反覆:讀本地跑兩層篩選 → reports/screener_result.md(快,可隨時改門檻重跑)
python screen.py
```

**第一層 資格篩選(6 條全過才進池,排地雷):** ①上市滿5年 ②近5年≥4年EPS正 ③近3年累積OCF為正且至少2年全年OCF為正 ④有息負債比低於產業門檻(金融股依設定排除)⑤近60日日均成交金額>門檻 ⑥有最新財報。

**第二層 品質篩選(通過第一層者中標記,不淘汰):** ⑦近3年營收CAGR>10% ⑧近3年毛利率斜率≥0 ⑨近3年ROE均>15% ⑩盈餘修正動能(**僅標記,不列入「兩層全過」判定**,依原則等回測驗證後才加權)。

**估值旗標層(只加旗標、不淘汰任何標的):** 歷史位階使用**目前 trailing PE 對近5年 trailing PE 分布**。🟢=前瞻PEG<1 且 trailing PE<歷史中位;🔴=trailing PE>歷史P90,或前瞻PEG>2,或前瞻PE>60x。forward與trailing分欄,不再混用口徑。

**美股(測試):** `config` 的 `extra_us`(如 `TSLA`)用 **yfinance** 跑同一套兩層篩選 + 估值旗標,`market` 參數切換台股(FinMind)/美股(yfinance)。

`reports/screener_result.md` 內容:門檻摘要、第一層漏斗統計(每條刷掉幾檔)、第一層通過清單(附⑦⑧⑨達標 ✅/❌/⚠️資料不足 + 代號/名稱/產業/CAGR/毛利趨勢/ROE/動能)、**兩層全過精華清單**、誠實說明。

- **主要門檻寫在 `config/screener.yaml`**;少數工程/訊號門檻仍在程式內(詳見 `SYSTEM_OVERVIEW.md`)。改篩選門檻後可直接重跑 `screen.py`。
- **誠實原則**:任一條件因資料缺失無法判斷 → 標「⚠️資料不足」,**一律不當通過**。
- **省請求**:`deep_fetch_only_liquid` 讓未達流動性者不深抓財報;`refetch_after_days` 讓抓取**可續跑**。
- **額度**:免費版逐檔抓全市場較久,強烈建議設 `FINMIND_TOKEN`;`data/universe/` 會進版控,作為 GitHub Actions 的資料快照。
