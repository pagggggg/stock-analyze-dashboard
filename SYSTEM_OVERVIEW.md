...
- 週二至週六台灣約 06:17 只抓完成交易日與 Reported EPS 事件，交易式同步 16 檔美股行情、7 檔美股母體 records 與 7 檔 AI 額外節點；不重抓年度財報，並更新 ASML、AMAT、LRCX、KLAC 河流圖。使用 `--no-record`，不重複寫入台股訊號。
- 週日 03:47 執行完整美股財報與共識更新，避免價格專用流程讓基本面資料永久不更新；台股財報維持每日分批輪替，避免一次強制重抓超過 FinMind 額度。
- GitHub 排程不保證準點，可能因平台佇列延遲數小時；頁面分開標示建置時間、台股行情時間與美股行情時間。
...
- partial/cache regression 依代號集合比較，不只比較總數；任一新缺漏都會擋下部署。
- 連續兩次品質略過，或任一市場落後超過一個實際交易日，會透過獨立健康 job 發出一次失敗通知；狀態保存在 Release asset，恢復後解除。
